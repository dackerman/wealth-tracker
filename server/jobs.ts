...
async function syncPlaidData() {
  try {
    ...
    const plaidInstitutions = institutions.filter(
      (institution) => institution.plaidInstitutionId && institution.accessToken
    );
    ...
    for (const institution of plaidInstitutions) {
      ...
      try {
        ...
        for (const accountData of accountsData) {
          ...
          if (existingAccount) {
            ...
            await storage.updateAccount(existingAccount.id, {
              ...
              availableBalance: accountData.balances.available?.toString() || '0',
        ...
        const allTransactions = await storage.getTransactionsByUserId(institution.userId);
        ...
        const allAccounts = await storage.getAccountsByUserId(institution.userId);
...
export function startBackgroundJobs() {
  ...
  // Also run immediately on startup for testing (can be removed in production)
  // syncPlaidData().catch(error => {
  //   log('Failed to run initial Plaid sync job', 'jobs');
  //   console.error('Initial sync error:', error);
  // });
  
  log('Background jobs scheduled', 'jobs');
  
  // Return functions for manual execution if needed
  return {
    syncPlaidData,
    updateNetWorth
  };
