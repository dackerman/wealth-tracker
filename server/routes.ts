...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/plaid/create-link-token", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create link token" });
    }
  });
  ...
  app.post("/api/accounts/manual", requireAuth, async (req, res) => {
    try {
      ...
      const existingInstitutions = await storage.getInstitutionsByUserId(
        req.body.userId,
      );
      let institution = existingInstitutions.find(
        (i) =>
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to add manual account" });
    }
  });
      ...
      try {
        ...
        const accountPromises = accountsData.map(async (accountData) => {
          return storage.createAccount({
            ...
            isoCurrencyCode:
              accountData.balances.iso_currency_code || undefined,
          });
        });
        ...
        const transactionPromises = transactionsData.map(
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to exchange public token" });
      }
    },
  );
  ...
  app.get("/api/institutions", requireAuth, async (req, res) => {
    try {
      const institutions = await storage.getInstitutionsByUserId(
        req.body.userId,
      );
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get institutions" });
    }
  });
  ...
  app.get("/api/accounts", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get accounts" });
    }
  });
  ...
  app.get("/api/institutions/:id/accounts", requireAuth, async (req, res) => {
    try {
      ...
      res.status(500).json({
        error: error.message || "Failed to get accounts for institution",
      });
    }
  });
  ...
  app.get("/api/transactions", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get transactions" });
    }
  });
  ...
  app.get("/api/accounts/:id/transactions", requireAuth, async (req, res) => {
    try {
      ...
      const limit = req.query.limit
        ? parseInt(req.query.limit as string, 10)
        : undefined;
      const transactions = await storage.getTransactionsByAccountId(
        accountId,
        limit,
      );
      res.json(transactions);
      ...
      res.status(500).json({
        error: error.message || "Failed to get transactions for account",
      });
    }
  });
  ...
  app.get("/api/net-worth/history", requireAuth, async (req, res) => {
    try {
      ...
      const history = await storage.getNetWorthHistoryByUserId(
        req.body.userId,
        limit,
      );
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get net worth history" });
    }
  });
  ...
  app.get("/api/net-worth/summary", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get net worth summary" });
    }
  });
  ...
  app.post("/api/refresh", requireAuth, async (req, res) => {
    try {
      ...
      for (const institution of institutions) {
        ...
        const updatedAccounts = await storage.getAccountsByInstitutionId(
          institution.id,
        );
        ...
        for (const transactionData of transactionsData) {
          ...
          const existingTransactions = await storage.getTransactionsByAccountId(
            account.id,
          );
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to refresh data" });
    }
  });
  ...
  app.post("/api/accounts/manual", requireAuth, async (req, res) => {
    try {
      ...
      let institution = (
        await storage.getInstitutionsByUserId(req.body.userId)
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create manual account" });
    }
  });
  ...
  app.post("/api/admin/jobs/sync-plaid-data", requireAuth, async (req, res) => {
    try {
      ...
      if (req.body.userId !== 1) {
        return res
          .status(403)
          .json({ error: "Not authorized to run admin jobs" });
      }
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to run manual Plaid sync" });
    }
  });

  app.post(
    "/api/admin/jobs/update-net-worth",
    requireAuth,
    async (req, res) => {
      try {
        // For security, only allow userId 1 (admin) to trigger jobs manually
        ...
        res.status(500).json({
          error: error.message || "Failed to run manual net worth update",
        });
      }
    },
  );
